...
public class BatteryPickupXRay : MonoBehaviour
{
    public GameObject objectToDestroy; // 需要销毁的物体（如空气墙）
    public GameObject batteryObject; // 需要显示的物体（Tag 为 "Battery3"）
    ...
    {
        // 玩家（手）碰到 Battery3
        if ((other.CompareTag("Left Hand") || other.CompareTag("Right Hand")) && !isPickedUp && CompareTag("Battery3"))
        {
            isPickedUp = true;
            Debug.Log("玩家拾取了 Battery3");

            // 显示 UI 并更新文本
            if (uiCanvas != null && uiText != null)
            {
                uiCanvas.SetActive(true);
                uiText.text = "This is an X-ray battery.";
            }
        ...
        // Battery3 碰到 Detection
        if (other.CompareTag("Detection") && isPickedUp)
        {
            Debug.Log("Battery3 碰到了 Detection");
            ...
            if (modeManager != null)
            {
                modeManager.UnlockXRayMode();
            }

            // 立即销毁 Battery3
            Destroy(gameObject);
            Debug.Log("Battery3 已销毁");

            // 更新 UI 文本并显示 UI Image
            if (uiCanvas != null && uiText != null)
            {
                uiText.text = "I need to tune the frequency to 3 × 10¹⁶ – 3 × 10¹⁹ Hz.";
